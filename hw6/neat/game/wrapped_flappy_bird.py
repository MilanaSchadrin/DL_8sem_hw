...
FONT_NAMES = pygame.font.SysFont("Roboto Condensed", 15)
FONT_SCORE = pygame.font.SysFont("Roboto Condensed Bold", 30)
FONT_HEADING = pygame.font.SysFont("Roboto Condensed Semibold Italic", 30)
...
class GameState:
    ...
    def frame_step(self, input_actions):
        ...
        reward = 0.1  # небольшое поощрение за каждый кадр
        terminal = False
        ...
        for pipe in self.upperPipes:
            ...
            if pipeMidPos <= playerMidPos < pipeMidPos + 4:
                self.score += 1
                reward = 10.0  # сильная награда за прохождение

                #angle_bonus = min(0, 5 - abs(self.playerRot) * 0.1)
                #reward -= angle_bonus
        ...
        if isCrash:
            terminal = True
            reward = -200.0  # сильное наказание за смерть
        ...
        if self.render:
            ...
            score_surface = FONT_SCORE.render(f"Score: {self.score}", True, (255, 255, 255))
            score_rect = score_surface.get_rect()
            score_rect.topleft = (10, 10)
            bg_rect = pygame.Surface((score_rect.width + 10, score_rect.height + 4))
            bg_rect.set_alpha(100)
            bg_rect.fill((0, 0, 0))
            SCREEN.blit(bg_rect, (score_rect.left - 5, score_rect.top - 2))
