...
class GameState:
    ...
    def frame_step(self, input_actions):
        ...
        vertical_distance = abs(player_center - pipe_center)
        max_vertical_distance = SCREENHEIGHT / 2
        vertical_alignment = 1.0 - (vertical_distance / max_vertical_distance)
        reward += 0.5 * vertical_alignment  # Награда за приближение к центру
    
    
        horizontal_distance = next_pipe['x'] - self.playerx
        horizontal_proximity=0.0
        if horizontal_distance > 0: 
            horizontal_proximity = 1.0 - min(horizontal_distance / 200, 1.0)
        reward += 0.1 * horizontal_proximity * vertical_alignment
        ...
        for pipe in self.upperPipes:
            ...
            if pipeMidPos <= playerMidPos < pipeMidPos + 4:
                self.score += 1
                alignment_bonus = 2.0 * vertical_alignment
                score_bonus = min(self.score * 0.2, 5.0)
                reward += 6.0 + alignment_bonus + score_bonus
        ...
        if isCrash:
            terminal = True
            if self.playery <= 0 or self.playery + PLAYER_HEIGHT >= BASEY:
                reward = -15.0
            else:
                next_pipe_idx = 0
                ...
                gap_y_center = upper_pipe['y'] + PIPE_HEIGHT + (lower_pipe['y'] - (upper_pipe['y'] + PIPE_HEIGHT)) / 2


                vertical_distance = abs(self.playery + PLAYER_HEIGHT / 2 - gap_y_center)

                max_distance = SCREENHEIGHT / 2
                proximity_factor = round(1 - min(vertical_distance / max_distance, 1.0),2)

                reward = -2 + proximity_factor  # ближе к центру — меньше штраф
